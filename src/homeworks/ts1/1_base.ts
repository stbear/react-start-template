...
export const getNumberedArray = (arr: string[]) => arr.map((value, number) => ({ value, number }));
...
export const transformCustomers = (customers: (Customer & {id: number})[]) => {
  return customers.reduce((acc: { [index: number]: Customer }, customer) => {
